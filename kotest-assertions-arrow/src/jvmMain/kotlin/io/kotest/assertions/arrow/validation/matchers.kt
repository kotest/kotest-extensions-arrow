...
public fun Validated<*, *>.shouldBeValid() {
  contract {
...
public fun <A> Validated<*, A>.shouldNotBeValid(): Unit =
  shouldNot(beValid())
...
public infix fun <A> Validated<*, A>.shouldBeValid(value: A): Unit =
...
public infix fun <A> Validated<*, A>.shouldNotBeValid(value: A): Unit =
  shouldNot(beValid(value))

@Deprecated("Convenience function is deprecated us shouldBeValid from core")
public infix fun <A> Validated<*, A>.shouldBeValid(fn: (Valid<A>) -> Unit) {
  this.shouldBeValid()
  fn(this)
}

@Deprecated("Use shouldBeValid from core directly")
public fun <A> beValid(): Matcher<Validated<*, A>> =
...
public fun <A> beValid(a: A): Matcher<Validated<*, A>> =
  object : Matcher<Validated<*, A>> {
    override fun test(value: Validated<*, A>): MatcherResult =
...
@OptIn(ExperimentalContracts::class)
public fun Validated<*, *>.shouldBeInvalid(): Unit {
  contract {
...
public infix fun <E> Validated<E, *>.shouldBeInvalid(value: E): Unit =
  should(beInvalid(value))

@Deprecated("Use shouldBeInvalid instead")
public infix fun <E> Validated<E, *>.shouldBeInvalid(fn: (Invalid<E>) -> Unit): Unit {
  this.shouldBeInvalid()
  fn(this)
}

@Deprecated("Use shouldBeInvalid directly")
public fun <E> beInvalid(): Matcher<Validated<E, *>> =
...
public fun <E> beInvalid(e: E): Matcher<Validated<E, *>> =
  object : Matcher<Validated<E, *>> {
    override fun test(value: Validated<E, *>): MatcherResult =
