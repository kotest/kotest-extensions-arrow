...
fun <T> NonEmptyList<T>.forAtMostOne(f: (T) -> Unit): Unit = forAtMostOne(f)

@Deprecated(
  "Use forAtMost from core",
  ReplaceWith("forAtMost(k, fn)", "io.kotest.assertions.arrow.core.forAtMost")
)
fun <T> NonEmptyList<T>.forAtMost(k: Int, fn: (T) -> Unit): Unit = forAtMost(k, fn)
...
fun <T> NonEmptyList<T>.forNone(f: (T) -> Unit): Unit = forNone(f)
