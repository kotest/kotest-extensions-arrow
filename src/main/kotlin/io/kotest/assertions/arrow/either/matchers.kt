...
inline infix fun <B> Either<*, B>.shouldBeRight(fn: (B) -> Unit) {
  this should beRight()
  fn((this as Either.Right<B>).value)
}
...
fun <B> beRight(b: B) = object : Matcher<Either<Any?, B>> {
  override fun test(value: Either<Any?, B>): MatcherResult {
    return when (value) {
      is Either.Left -> {
        MatcherResult(false, "Either should be Right($b) but was Left(${value.value})", "Either should not be Right($b)")
      }
      is Either.Right -> {
        if (value.value == b)
          MatcherResult(true, "Either should be Right($b)", "Either should not be Right($b)")
        else
          MatcherResult(false, "Either should be Right($b) but was Right(${value.value})", "Either should not be Right($b)")
      }
    }
  }
}
...
inline infix fun <A> Either<A, *>.shouldBeLeft(fn: (A) -> Unit) {
  this should beLeft()
  fn((this as Either.Left<A>).value)
}
...
fun <A> beLeft(a: A) = object : Matcher<Either<A, Any?>> {
  override fun test(value: Either<A, Any?>): MatcherResult {
    return when (value) {
      is Either.Right -> {
        MatcherResult(false, "Either should be Left($a) but was Right(${value.value})", "Either should not be Right($a)")
      }
      is Either.Left -> {
        if (value.value == a)
          MatcherResult(true, "Either should be Left($a)", "Either should not be Left($a)")
        else
          MatcherResult(false, "Either should be Left($a) but was Left(${value.value})", "Either should not be Right($a)")
      }
    }
  }
}
...
inline fun <reified A> beLeftOfType() = object : Matcher<Either<Any?, Any?>> {
  override fun test(value: Either<Any?, Any?>): MatcherResult {
    return when (value) {
      is Either.Right -> {
        MatcherResult(false, "Either should be Left<${A::class.qualifiedName}> but was Right(${value.value})", "")
      }
      is Either.Left -> {
